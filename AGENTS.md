...
微信悬浮窗助手（macOS）：OCR 读目标 IM 窗口（个人微信或企业微信）→ 本地模型判意图/风险 → LLM 生成候选回复 → 悬浮窗展示/一键填入。纯只读、零封号风险是**核心原则**，任何改动不得破坏。
...
- **纯只读**：不注入、不 hook、不解密目标 IM 数据。「填入」是唯一写动作：AX 写入优先；目标 IM 不提供 AX 输入控件时（如微信 4.1 部分环境），显式点击「填入」走 `visual_fill` 视觉兼容路径——点击输入区 + CGEvent 键盘事件，**不发送、不用剪贴板/Cmd+V、不覆盖草稿、窗口/焦点/会话签名三重复核、OCR 读回确认、失败不自动重试**（README「输入区检测框与填入」）——**别改回剪贴板+模拟 Cmd+V**（切前台不可靠、覆盖剪贴板、失败会贴进别的应用，见 `src/fill.py` 顶部注释）。
