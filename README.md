# jev-chat-jarvis（macOS） 增加企业微信支持

微信或企业微信弹出一条消息 → 悬浮窗立刻告诉你**这句话的真实意图**、**风险几级**、**该怎么回**。
...
- **YOLO 检测框**（可选，`JEV_BOXES=1` 启动即开、菜单栏可切）：OCR 命中的消息实时框在目标 IM 窗口上，对方/我分色 + 置信度
...
**从源码跑**（微信或企业微信在运行、终端已授予屏幕录制）：`./start.command`。分层自测：
...
- 悬浮窗在**微信或企业微信**位于前台时显示和分析；切到浏览器等其他应用会立即隐藏并丢弃未完成的旧结果，返回当前 IM 后强制重新读取当前聊天，避免把旧面板误认为正在识别其他应用
...
- IM 改版会让布局常量失效（`src/app_profile.py` 各 profile 顶部常量需重新校准）。个人微信按 **4.1** 标定；企业微信身份已在 **5.0.11**（owner `WeCom`，bundle `com.tencent.WeWorkMac`）标定，**布局常量仍为临时抄值**（与个微 4.1 相同），需开 `JEV_BOXES=1` 并在授予屏幕录制权限下跑 HUD 检测框后重校准；窗标题与填入标题别名亦待 Screen Recording 下复探。多窗口优先识别主窗口；名字里含「微信」的兄弟应用（微信读书、微信输入法）不参与窗口选择
- 本地判断模型首次下载时，悬浮窗状态行显示模型权重的大致下载进度和体积（如「下载判断模型 34% · 1.2/3.8 GB」，以实际下载文件为准；分块处理时进度可能跳升）；完成后进入加载/预热，再恢复消息状态。已有缓存时不显示下载进度；Jev 不可用转本地时同样适用。
...
- 架构一句话：目标 IM（微信或企业微信）在前台时，进程内抓其窗口 → Vision OCR（只扫聊天区）→ 本地 decider-2b 出意图/风险 → LLM 并发出候选 → 本地排序 → 悬浮窗 NSPanel。底层仍按窗口 ID 抓取而不是全屏截图，悬浮窗不污染 OCR

## 真机自测清单（企业微信）

| # | 步骤 | 期望 | 状态 |
|---|------|------|------|
| 1 | 仅微信前台 | 与改前一致：读屏、候选、填入 | 已通过（历史口径） |
| 2 | 仅企微前台 | 检测框准；候选出现；填入读回 OK | **待验证**（布局未校准） |
| 3 | 微信 → Chrome | 面板隐藏 | **待验证** |
| 4 | Chrome → 企微 | 强制重读，无微信残留 | **待验证** |
| 5 | 企微 ↔ 微信 | 清缓存、不串窗、不串会话标题 | **待验证** |
| 6 | 微信读书前台 | 面板隐藏，不 OCR | **待验证** |

企微检测框对齐与 AX/视觉填入需在 `JEV_BOXES=1` + 屏幕录制权限下人工验收后再标为完成。

## 许可与免责

MIT（见 `LICENSE`）。只读**你自己屏幕上、你自己账号的**聊天内容，不注入、不 hook、不解密数据库、不自动发送任何消息。请在自己设备上自用；装到别人机器上读别人的聊天记录是另一回事，本项目不为那种用法背书。IM 改版可能导致布局识别失效，请遵守相应软件许可协议。
